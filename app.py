"""
OS Insider Scanner — Streamlit App (v3.3)
2 settori: Biotech + Semiconductors
Prefiltro nome azienda → drastica riduzione SIC lookup
"""
...
def run_scan_full(progress_ph, log_ph, soglia, filter_clevel, parse_concurrency):
    ...
    if not all_filings:
        return {"error": "Full-index non disponibile (di solito dopo le 22:00 ET)"}
    ...
    # 3. PREFILTRO NOME — riduce drasticamente le SIC lookup
    # Aziende con nome che NON contiene keyword bio/semi vengono skippate
    # SENZA chiamare data.sec.gov
    candidates = [f for f in new_filings if edgar.is_candidate_company_name(f["company"])]
    skipped_byname = len(new_filings) - len(candidates)
    add_log(f"Prefiltro nome: {len(candidates)}/{len(new_filings)} candidati (skip {skipped_byname} per nome)")
    
    # Marca COMUNQUE tutti come processati per non riguardarli
    for f in new_filings:
        db_mark_processed(conn, f["accession"])
    conn.commit()
    
    if not candidates:
        return {"new_matches": 0, "elapsed": time.time() - t0, "msg": "Nessun candidato dopo prefiltro nome"}
    
    # 4. SIC lookup SOLO sui candidati (sequenziale per data.sec.gov)
    ...
    if ciks_to_fetch:
        progress_bar = progress_ph.progress(0.0, text=f"SIC lookup 0/{len(ciks_to_fetch)} (sequenziale)")
        
        # SEQUENZIALE per data.sec.gov per evitare 429
        for i, cik in enumerate(ciks_to_fetch, 1):
            ...
            if i % 5 == 0 or i == len(ciks_to_fetch):
                conn.commit()
                progress_bar.progress(i / len(ciks_to_fetch), text=f"SIC lookup {i}/{len(ciks_to_fetch)}")
        
        progress_bar.empty()
    
    # 5. Filtro candidati nei 2 settori
...
def run_refresh_atom(progress_ph, log_ph, soglia, filter_clevel, parse_concurrency):
    """Refresh ATOM con prefiltro nome."""
    conn = get_db()
    session = edgar.make_session(get_email())
    ...
    # Prefiltro su nome (dal feed ATOM ho il title = nome azienda)
    ...
    if not candidates:
        return {"new_matches": 0, "elapsed": time.time() - t0, "msg": "Nessun candidato dopo prefiltro"}
    
    # Parse in parallelo
    progress_bar = progress_ph.progress(0.0, text=f"Parsing 0/{len(candidates)}")
    ...
    results = edgar.parallel_map(parse_task, candidates, max_workers=parse_concurrency, progress_callback=cb)
    progress_bar.empty()
    
    # SIC lookup solo per quelli con purchases
    with_purchases = [(f, p, u) for f, p, u in (r for r in results if r) if p and p["purchases"]]
    ciks = list(set(p["issuer_cik"] for _, p, _ in with_purchases))
    ...
    for filing, parsed, doc_url in with_purchases:
...
st.caption("Form 4 P+A · Biotech + Semiconductors · 24h coverage")
...
with st.sidebar:
    ...
    parse_concurrency = st.select_slider(
        ...
        options=[3, 5, 8],
        value=5,
        help="Parallelizza solo www.sec.gov (data.sec.gov resta sequenziale)"
    )
    ...
    st.divider()
    st.subheader("🔬 Prefiltro nome")
    st.caption("Aziende con nome che NON contiene keyword bio/pharma/semi/chip vengono skippate "
               "senza chiamare SEC. Riduce ~80% delle SIC lookup.")
...
if len(df) == 0:
    ...
    disp["🏢 Ticker"] = disp.apply(
        lambda r: f"⚡{r['ticker']} ({r['cluster_size']})" if r["is_cluster"] else r["ticker"], axis=1
    )
    disp["👤 Insider"] = disp["insider_name"].apply(lambda x: (x or "")[:28])
    disp["Ruolo"] = disp["insider_title"].apply(lambda x: (x or "")[:25])
    
    st.dataframe(
        disp[["🏢 Ticker", "company", "👤 Insider", "Ruolo", "tx_date",
...
st.divider()
st.caption(f"💾 Cache SIC: {stats['sic_cache']:,} · Processati: {stats['processed']:,} · v3.3")
